...
import {
  getMailActionDetailsUser, postMailActionDetailsUser
} from "app/modules/administration/user-management/developer-settings/developer-settings.reducer";

const EditEmailConfigurationUser = ({mailActionId, showModal, handleClose, mailActionDetails}) => {
  const [errors, setErrors] = useState<{ contentError?: string; mailToError?: string; }>({});
  const dispatch = useAppDispatch();
  const mailActionDetailsPayload = useAppSelector(state => state.developerSettings.mailActionDetailsPayload);
  ...
  useEffect(() => {
    if (showModal) {
      dispatch(getMailActionDetailsUser(mailActionId))
    }
  }, [showModal])
  ...
  useEffect(() => {
    if (mailActionDetailsPayload !== undefined) {
      ...
      const outputArray = mailActionDetailsPayload?.selectedMailToVariables?.map(value => ({value, label: value}));
      setMailTo(outputArray)
      setSubject(mailActionDetailsPayload?.subject)
    }
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      dispatch(postMailActionDetailsUser(data))
    }
  }
      ...
      <div className="d-flex mt-3 Submittd_list" style={{border: 'none !important', height: '600px'}}>
        <div className="col-3 px-3 position-relative"
          ...
          <ul className="list-group mt-3">
            ...
            {mailActionDetailsPayload?.commonMailActionVariables?.length !== 0 ? mailActionDetailsPayload?.commonMailActionVariables?.map((keyword, index) => (
...
export default EditEmailConfigurationUser;
