...
const AssignCopyEditing = (props) => {
  const dispatch = useAppDispatch();
  const {showModal, handleClose,setParticipantList,participantList,submissionId} = props;
  const assignUserSuccess = useAppSelector(state => state.copyEditing.assignUserSuccess);
  const postAssignUserSuccessPayload = useAppSelector(state => state.copyEditing.postAssignUserSuccessPayload);
  ...
  const HandleSubmit = () => {
    ...
    if (validateForm()) {
      dispatch(postAssignUserCopyEditing(data));
    }
  }
...
export default AssignCopyEditing;
