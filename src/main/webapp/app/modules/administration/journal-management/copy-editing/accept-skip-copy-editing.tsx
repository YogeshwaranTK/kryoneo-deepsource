...
import {movetoProduction, postCopyEditingEditedFileUpload, stateFalse} from "./copy-editing.reducer";
...
const AcceptSkipCopyEditing = (props) => {
  const dispatch = useAppDispatch();
  const {
    showModal,
    handleClose,
    copyEditingDraftFilesList,
    submissionId
  } = props;
  const moveToProductionSuccess = useAppSelector(state => state.copyEditing.moveToProductionSuccess);
  ...
  useEffect(() => {
    setFile(copyEditingDraftFilesList)
  }, [copyEditingDraftFilesList])
  ...
  useEffect(() => {
    if (moveToProductionSuccess) {
      navigate(`/journal/${Jo_id}/copyeditings`)
      //   dispatch(stateFalse())
    }

  }, [moveToProductionSuccess])
  ...
  useEffect(() => {
    ...
    if (fileUploadCopyEditedFileSuccess) {
      const uploadfile = postFileUploadCopyEditedFilePayload.map(item => item.id)
      setSelectFile([...selectFile, ...uploadfile])
      setShowUploadFile(false);

    }
  }, [fileUploadCopyEditedFileSuccess]);
          ...
          <div className="modal-body ps-4 position-relative">
            ...
            <div className="pt-3">
              <div className="d-flex justify-content-between align-items-center">
                <div>
                  <label className="form-label">Choose the File</label>
                </div>
            ...
            {showUploadFile && <>
              ...
              <div className="modal-footer justify-content-end">
                ...
                <button type={"submit"} onClick={HandleFileSubmit} className="btn btn--cancel">
                  UPLOAD FILE
                </button>
              </div>
            </>
...
export default AcceptSkipCopyEditing;
