...
import { postAssignUserReview } from "./reviewReducer";
...
const ReviewAssignProduction = (props) => {
  const dispatch = useAppDispatch();
  const { showModal, handleClose, setParticipantList, participantList, submissionId, reviewRoundId } = props;

  const assignUserSuccess = useAppSelector(state => state.review.assignUserSuccess);
  const postAssignUserSuccessPayload = useAppSelector(state => state.review.postAssignUserSuccessPayload);
  ...
  const HandleSubmit = () => {
    ...
    if (validateForm()) {
      dispatch(postAssignUserReview(data));
    }
  }
  ...
  const fetchSuggestions = async (value: string) => {
    ...
    try {
      ...
      } else if (selectReviewer.value === "EDITORIAL") {
    ...
    <Modal
      ...
      <div className="modal-content">
        ...
        <div className="modal-body ps-4 position-relative">
          ...
          <div className="pt-3">
            <label className="form-label">Select Reviewer type<span className="error_class">*</span>
...
export default ReviewAssignProduction;
