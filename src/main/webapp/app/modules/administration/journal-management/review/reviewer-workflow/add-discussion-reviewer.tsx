...
import {
  getReviewRoundContributor,
  postAddDiscussionReviewerRequest,
  getDiscussionReviewerRequest,
  getDiscussionReviewerRequestRaw, stateReviewerFalse
...
const AddDiscussionReviewer = (props) => {
  const dispatch = useAppDispatch();
  const {
    showModal,
    handleClose,
    submissionId,
    setReviewRoundDiscussions,
  } = props;
  const addDiscussionReviewerSuccess = useAppSelector(state => state.reviewer.addDiscussionReviewerSuccess);
  const reviewerSubmissionSingleDetails = useAppSelector(state => state.reviewer.reviewerSubmissionSingleDetails);
  const getReviewRoundContributorDetails = useAppSelector(state => state.reviewer.getReviewRoundContributorDetails);
  ...
  useEffect(() => {
    if (addDiscussionReviewerSuccess === true) {
      const data = {
        reviewRoundReviewerId,
        reviewRoundId
      }
      setLocalLoading(true);
      getDiscussionReviewerRequestRaw(data)
        .then(response => {
          setReviewRoundDiscussions(response.data)
  ...
  const HandleSubmit = () => {
    ...
    if (validateForm()) {
      dispatch(postAddDiscussionReviewerRequest(data));
    }
  }
  ...
  useEffect(() => {
    if (showModal === true) {
      ...
      dispatch(getReviewRoundContributor(reviewRoundId));
      dispatch(getDiscussionReviewerRequest({reviewRoundReviewerId,reviewRoundId}));
    }
  }, [showModal])
      ...
      <Modal
        ...
        <div className="modal-content">
          ...
          <div className="modal-body ps-4 position-relative">
            ...
            {loading || localLoading ? <LoaderMain/> : null}
            ...
            {
              getReviewRoundContributorDetails?.map((value) => (
...
export default AddDiscussionReviewer;
