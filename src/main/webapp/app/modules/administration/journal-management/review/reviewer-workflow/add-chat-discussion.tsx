...
import {
  getDiscussionReviewerRequestRaw,
  postAddDiscussionChatReviewerRequest, stateReviewerFalse
...
const AddChatReviewer = (props) => {
  const dispatch = useAppDispatch();
  const {showModal, handleClose, submissionId, selectDiscussionId, setReviewRoundDiscussions} = props;
  ...
  const addDiscussionChatReviewerSuccess = useAppSelector(state => state.reviewer.addDiscussionChatReviewerSuccess);
  ...
  useEffect(()=>{
    if (addDiscussionChatReviewerSuccess){
      const data = {
  ...
  },[addDiscussionChatReviewerSuccess]);
  ...
  const HandleSubmit = () => {
    if (validateForm()) {
      ...
      dispatch(postAddDiscussionChatReviewerRequest(requestData));
    }
  }
      ...
      <Modal
        ...
        <div className="modal-content">
          ...
          <div className="modal-body ps-4 position-relative">
            ...
            {loading || isLoading || localLoading ? <LoaderMain/> : null}
...
export default AddChatReviewer;
