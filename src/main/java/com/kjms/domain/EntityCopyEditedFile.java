...
@Entity
@Table(name = "jm_copy_edited_file")
public class EntityCopyEditedFile {
    ...
    @JoinColumn(name = "submission_id", foreignKey = @ForeignKey(name = "fk_jm_copy_edited_file_submission_id"),nullable = false)
    private EntitySubmission submission;

    @ManyToOne(cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    @JoinColumn(name = "journal_id", foreignKey = @ForeignKey(name = "fk_jm_copy_edited_file_journal_id"),nullable = false)
    private EntityJournal journal;
    // TODO: 13-Nov-23 @varghesh (column name _id missing)
    @ManyToOne(cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    @JoinColumn(name = "file_type", foreignKey = @ForeignKey(name = "fk_jm_copy_edited_file_file_type_id"),nullable = false)
    ...
    @ManyToOne(cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    @JoinColumn(name = "created_by", foreignKey = @ForeignKey(name = "fk_jm_copy_edited_file_created_by"), updatable = false, nullable = false)
