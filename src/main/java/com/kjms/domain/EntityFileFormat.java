...
/**
 * Entity class representing an Article Submission File Format.
 */
@Entity
@Table(name = "jm_submission_file_format")
public class EntityFileFormat {
    ...
    @JoinColumn(name = "created_by", foreignKey = @ForeignKey(name = "fk_jm_submission_file_format_created_by"), updatable = false, nullable = false)
    ...
    @ManyToOne(cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    @JoinColumn(name = "last_modified_by", foreignKey = @ForeignKey(name = "fk_jm_submission_file_format_last_modified_by"))
